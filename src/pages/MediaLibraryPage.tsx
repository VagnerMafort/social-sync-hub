...
import { Upload, Film, ImageIcon, Layers, Search, Filter, Trash2, MoreHorizontal } from 'lucide-react';
    ...
    newUploads.forEach((_, i) => {
      ...
      const interval = setInterval(() => {
        ...
        if (progress >= 100) {
          ...
          setTimeout(() => {
            setUploadingFiles((prev) => prev.filter((f) => f.name !== acceptedFiles[i].name));
          }, 1000);
        }
        setUploadingFiles((prev) =>
          prev.map((f) => (f.name === acceptedFiles[i].name ? { ...f, progress } : f))
        );
      }, 300);
    });
  }, []);
            ...
            <div key={item.id} className="card-elevated group overflow-hidden transition-shadow">
              <div className="relative aspect-square bg-muted overflow-hidden">
                ...
                <div className="absolute top-2 left-2">
                  <span className={mediaStatusChipClass(item.status)}>{item.status}</span>
                </div>
                <div className="absolute top-2 right-2">
                  <TypeIcon className="h-4 w-4 text-primary-foreground drop-shadow" />
                </div>
                {item.duration && (
