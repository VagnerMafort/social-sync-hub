...
import { mockQueue, mockPosts } from '@/lib/mock-data';
...
export default function QueuePage() {
  const [jobs] = useState<QueueJob[]>(mockQueue);

  const getPost = (postId: string) => mockPosts.find((p) => p.id === postId);
                ...
                return (
                  <tr key={job.id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-5 py-3">
                      <p className="text-foreground truncate max-w-[200px]">{post?.caption || job.post_id}</p>
                    </td>
                    <td className="px-5 py-3">
                      {post && <PlatformIcon platform={post.platform} className="h-4 w-4" />}
                    </td>
                    <td className="px-5 py-3">
                      <span className={queueStatusChipClass(job.status)}>
                        <StatusIcon className="h-3 w-3" /> {job.status}
                      </span>
                    </td>
