...
import { mockAnalytics, mockPosts, mockAccounts } from '@/lib/mock-data';
import { PlatformIcon, platformLabel } from '@/components/PlatformIcon';
import { postStatusChipClass } from '@/lib/helpers';
import { format } from 'date-fns';

const stats = [
  { label: 'Total Posts', value: mockAnalytics.total_posts, change: '+12%', up: true, icon: CalendarDays },
  { label: 'This Week', value: mockAnalytics.posts_this_week, change: '+3', up: true, icon: TrendingUp },
  { label: 'Total Views', value: `${(mockAnalytics.total_views / 1000).toFixed(1)}K`, change: '+8.2%', up: true, icon: Image },
  { label: 'Engagement', value: `${mockAnalytics.engagement_rate}%`, change: '-0.3%', up: false, icon: AlertTriangle },
];

export default function DashboardPage() {
  const recentPosts = mockPosts.slice(0, 5);
  const chartData = mockAnalytics.daily_posts.slice(-14);
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <div
                      ...
                      style={{ width: `${(p.views / mockAnalytics.total_views) * 100}%` }}
                    />
                  </div>
                </div>
              </div>
