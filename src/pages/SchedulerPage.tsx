...
import { mockPosts, mockAccounts, mockMedia } from '@/lib/mock-data';
...
export default function SchedulerPage() {
  ...
  const filteredAccounts = newPost.platform
    ? mockAccounts.filter((a) => a.platform === newPost.platform && a.status === 'connected')
    : [];
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center border rounded-lg overflow-hidden">
            <button
              ...
              className={cn('px-3 py-2 text-sm', view === 'calendar' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted')}
            >
              <CalendarDays className="h-4 w-4" />
            </button>
            <button
              onClick={() => setView('list')}
              className={cn('px-3 py-2 text-sm', view === 'list' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted')}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  ...
                  {post.hashtags.length > 0 && (
                    <span className="ml-2 text-primary">{post.hashtags.slice(0, 3).join(' ')}</span>
                  )}
                </p>
              </div>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-2">
                ...
                <Select value={newPost.account_id} onValueChange={(v) => setNewPost((p) => ({ ...p, account_id: v }))} disabled={!newPost.platform}>
                  ...
                  <SelectContent>
                    {filteredAccounts.map((a) => (
                      <SelectItem key={a.id} value={a.id}>{a.platform_username}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={newPost.media_id} onValueChange={(v) => setNewPost((p) => ({ ...p, media_id: v }))}>
                ...
                <SelectContent>
                  {mockMedia.filter((m) => m.status === 'ready').map((m) => (
          ...
          <DialogFooter>
            ...
            <Button onClick={() => setShowDialog(false)}>Schedule Post</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
